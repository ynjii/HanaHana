...
public class Player : MonoBehaviour
{
    ...
    private void Swim()
    {
        if (SceneManager.GetActiveScene().name == Define.Scene.MerMaid.ToString()||SceneManager.GetActiveScene().name=="HyunminMermaid2"|| SceneManager.GetActiveScene().name == "MerMaid_YUJIN")
        {
            if (isWater)
            {
                jump_power = 6;
